...
writeFileSync(
  ...
  JSON.stringify({ ...ds.config, generatedAt: new Date().toISOString() }, null, 2)
);
...
console.log(
  'RB 主力后复权首值:',
  rb.mainAdj.find((x) => x != null).toFixed(2),
  ' 展期次数:',
  rb.rolls.length
);
console.log(
...
console.log(
  '持久化目录:',
  dataDir,
  withBars ? `(含全部合约日线 ${(barsBytes / 1048576).toFixed(1)} MB)` : '(连续序列 + 元数据)'
);
