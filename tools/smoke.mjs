...
if (!MT) {
  console.error('FAIL: MockTrader global not set');
  process.exit(1);
}
...
const ok =
  Math.abs(ma.annualizedReturn - mb.annualizedReturn) < 1e-12 &&
...
if (workerResult.type !== 'result') {
  console.error('FAIL worker:', workerResult.message);
  process.exit(1);
}
console.log(
  'worker OK: trades',
  workerResult.result.summary.nTrades,
  'annualized',
  workerResult.result.performance.metrics.annualizedReturn.toFixed(6)
);
...
for (const marker of ['id="chart"', 'id="runBtn"', 'id="verdict"', 'LineChart', 'MockTrader']) {
  if (!html.includes(marker)) {
    console.error('FAIL html missing marker:', marker);
    process.exit(1);
  }
}
if (html.includes('__WORKER_SOURCE__') || html.includes('__CORE__') || html.includes('__APP__')) {
  console.error('FAIL html has unreplaced placeholder');
  process.exit(1);
}
