...
(function (global) {
  ...
  class LineChart {
    ...
    setRange(range) {
      ...
      if (range === '1y') {
        this.view = { start: Math.max(0, n - 252), end: n - 1 };
      } else if (range === '3y') {
        this.view = { start: Math.max(0, n - 756), end: n - 1 };
      } else {
        this.view = null;
      }
      this.draw();
    ...
    _bind() {
      ...
      c.addEventListener('mousedown', (e) => {
        this.drag = { startX: e.offsetX, view0: this.view ? { ...this.view } : null };
      });
    ...
    _width() {
      return this.canvas.clientWidth || this.canvas.parentElement.clientWidth || 800;
    }
    _height() {
      return this.canvas.clientHeight || 320;
    }
    _pad() {
      return { left: 64, right: 18, top: 16, bottom: 26 };
    }

    _visibleSeries() {
      return this.series.filter((s) => this.visible[s.name] !== false);
    }
    ...
    _range() {
      ...
      if (this.view) {
        return [this.view.start, this.view.end];
      }
      return [0, n - 1];
    }
    ...
    draw() {
      ...
      for (let k = 0; k <= xTicks; k++) {
        ...
        ctx.stroke();
        if (this.dates[i]) {
          ctx.fillText(this.dates[i].slice(2), xx, h - pad.bottom + 5);
        }
      }
      ...
      if (this.showDrawdown && this.series.length) {
        ...
        for (let i = v0; i <= v1; i++) {
          if (dd[i]) {
            ...
            ctx.lineTo(x(i), pad.top);
            started = false;
          }
        }
        if (started) {
          ctx.lineTo(x(v1), pad.top);
        }
      ...
      for (const s of this.series) {
        ...
        ctx.setLineDash([]);
        ctx.fillStyle =
          this.visible[s.name] !== false ? 'rgba(220,235,255,0.9)' : 'rgba(120,130,150,0.6)';
