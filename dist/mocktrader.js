...
__def("index", function(__req, __exports) {
  ...
  const { generateReport, computeReportExtras } = __req("report/reportGenerator");
/**
...
__def("report/reportGenerator", function(__req, __exports) {
  const { FACTOR_KEYS, FACTOR_NAMES } = __req("types");
  const { historicalVaR, expectedShortfall, maxDrawdown, stressTest } = __req("risk/risk");
  const { quantileReturns, icDecay, topTurnover } = __req("research/factorAnalysis");
/**
...
function computeReportExtras(result, opts = {}) {
...
function generateReport(result, opts = {}) {
...
function buildMarkdown(j) {
  ...
  Object.assign(__exports, { computeReportExtras, generateReport });
});
