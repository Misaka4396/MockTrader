...
globalThis.assertGt = (a, b, msg = '') => { if (!(a > b)) throw new Error(`${msg} expected ${a} > ${b}`); };
globalThis.assertLt = (a, b, msg = '') => { if (!(a < b)) throw new Error(`${msg} expected ${a} < ${b}`); };
...
for (const t of tests) {
  ...
  catch (e) { fails.push({ name: t.name, err: e }); console.log('  FAIL  ' + t.name + ' -> ' + e.message); }
}
