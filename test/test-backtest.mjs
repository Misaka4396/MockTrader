...
function fakeDs({ prices, rolls = [], mult = 1, margin = 0.1, tickValue = 1 }) {
  ...
  return {
    ...
    getSeries: () => ({
      dates,
      mainAdj: prices.slice(), mainRaw: prices.slice(),
      subRaw: prices.slice(), mainOi: prices.map(() => 100), mainVol: prices.map(() => 100),
...
test('S4 long PnL sign + cost deduction (rising market)', () => {
  ...
  const bt = new BacktestEngine().run(ds, strategy, { initialCapital: 1000, executionDelay: 1, commissionRate: 0.0002, slippageTicks: 1 });
  ...
  assert(bt.trades.some((t) => t.side === 'open' && t.dir === 1), 'opened long');
});
...
test('S4 roll recorded + cost charged, no equity jump', () => {
  ...
  const ds = fakeDs({ prices, rolls: [{ date: '2022-01-05', from: 'XX001', to: 'XX002', fromClose: 101.5, toClose: 102 }] });
  const strategy = { rebalanceDates: [ds.dates[0]], targets: { [ds.dates[0]]: { XX: 1.0 } } };
  const bt = new BacktestEngine().run(ds, strategy, { initialCapital: 1000, executionDelay: 1, commissionRate: 0.0002, slippageTicks: 1 });
...
test('S4 margin cap respected', () => {
  ...
  const strategy = { rebalanceDates: [ds.dates[0]], targets: { [ds.dates[0]]: { XX: 1.0 } } };
  const bt = new BacktestEngine().run(ds, strategy, { initialCapital: 10000, executionDelay: 1, maxLeverage: 0.05, commissionRate: 0.0002, slippageTicks: 1 });
