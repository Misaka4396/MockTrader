...
import { METADATA } from '../src/core/data/metadata.js';

const da = new DataAccess();
da.generate({ start: '2022-01-03', end: '2023-12-29', varieties: ['RB', 'CU', 'M', 'AU', 'RS', 'WR', 'BB'] });
...
for (const code of da.codes) {
  ...
  console.log(`${code} ${meta.name} [${meta.sector}]`, {
    ...
    mult: meta.mult, margin: meta.margin, tickValue: meta.tickValue,
    list: meta.list, delist: meta.delist || 'active',
  });
}
...
for (let t = 0; t < rb.dates.length; t++) {
  if (rb.mainRaw[t] != null && rb.subRaw[t] != null && rb.mainCode[rb.dates[t]] && rb.subCode[rb.dates[t]]) {
...
console.log('RB avg annualized rollYield:', (acc / n).toFixed(4), ' (expect >0 since carryMean<0/backwardation)');

// back-adjust no-jump verification on raw vs adj
const cu = da.getSeries('CU');
let maxRaw = 0, maxAdj = 0;
