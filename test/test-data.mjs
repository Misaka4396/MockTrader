...
import { continuousSeries, buildMainSub, backAdjustFactors, buildIndex } from '../src/core/data/roll.js';
import { METADATA, METADATA_BY_CODE } from '../src/core/data/metadata.js';
...
test('S1 data reproducible (same seed => identical bars)', () => {
  const a = new DataAccess().generate({ start: '2022-01-03', end: '2023-12-29', varieties: ['RB', 'CU'] });
  const b = new DataAccess().generate({ start: '2022-01-03', end: '2023-12-29', varieties: ['RB', 'CU'] });
  ...
  assertClose(a.getBars('CU', 'CU2303')[10].close, b.getBars('CU', 'CU2303')[10].close, 1e-9, 'bar close');
});

// ---- 主力/次主力/各月合约可查 ----
test('S1 main/sub/monthly contracts queryable', () => {
  const da = new DataAccess().generate({ start: '2022-01-03', end: '2022-12-30', varieties: ['RB', 'M'] });
  const rb = da.getSeries('RB');
  assert(rb.mainRaw.some((x) => x != null), 'RB main present');
  assert(rb.subRaw.some((x) => x != null), 'RB sub present');
  const codes = da.getContracts('RB');
  assert(codes.length > 20, 'RB has many monthly contracts');
  assert(codes.every((c) => c.startsWith('RB')), 'contract codes prefixed');
  ...
  for (const r of rolls) assert(r.from !== r.to, 'roll changes contract');
});
...
test('S1 delisted varieties retained in metadata + partial data', () => {
  ...
  const da = new DataAccess().generate({ start: '2022-01-03', end: '2023-12-29', varieties: ['RS', 'BB', 'WR', 'RB'] });
  const rs = da.getSeries('RS');
  assert(rs.mainRaw.every((x) => x == null), 'RS has no data after delist (2021)');
...
test('S1 roll yield sign matches carry (backwardation>0, contango<0)', () => {
  const da = new DataAccess().generate({ start: '2022-01-03', end: '2023-12-29', varieties: ['RB', 'CU', 'AU', 'M', 'FG'] });
  const rollYieldMean = (code) => {
    ...
    for (let t = 0; t < s.dates.length; t++) {
      if (s.mainRaw[t] != null && s.subRaw[t] != null && s.subRaw[t] > 0) {
        const mc = s.mainCode[s.dates[t]]; const sc = s.subCode[s.dates[t]];
        if (!mc || !sc) continue;
        ...
        if (dt > 0) { acc += ((s.mainRaw[t] - s.subRaw[t]) / s.subRaw[t]) * (365 / dt); n++; }
      }
    }
    return acc / n;
  };
  ...
  for (const code of ['RB', 'M', 'FG']) {
    ...
    if (c < 0) assertGt(rollYieldMean(code), 0, code + ' roll yield >0 (backwardation)');
  }
  // carryMean>0 -> contango -> roll yield < 0
  for (const code of ['CU', 'AU']) {
    const c = SIM_OVERRIDES[code]?.carryMean ?? 0;
    if (c > 0) assertLt(rollYieldMean(code), 0, code + ' roll yield <0 (contango)');
  }
});
